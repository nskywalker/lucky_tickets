...
std::vector<uint64_t> LuckyTicketsSolver::calc_n_ticket(const uint64_t N)
{
    ...
    const auto prevAnsSize = prevAns.size();
    ...
    for (uint64_t i = 0; i < numberPerRow; ++i) {
        ...
        std::span row(begin, prevAnsSize);
        std::ranges::copy(prevAns, row.begin());
    }
    std::vector<uint64_t> result(elemsInRow, 0);
    for (uint64_t i = 0; i < elemsInRow; ++i) {
        auto col = temp
        ...
        result[i] = std::accumulate(col.cbegin(), col.cend(), 0ull);
    }
    return result;
}
